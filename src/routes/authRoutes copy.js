...
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    res.cookie("token", token, {
      httpOnly: true,
      secure: process.env.NODE_ENV === "production", // wajib diaktifkan di hosting HTTPS
      sameSite: "lax", // atau "none" jika frontend beda domain (pastikan juga pakai HTTPS)
...
*/
    res.cookie("token", token, {
      httpOnly: true,
      secure: false, // ⛔ Jangan true saat masih pakai localhost (tanpa HTTPS)
      sameSite: "lax",
      maxAge: 2 * 60 * 60 * 1000, // 2 jam
      path: "/",
    });
    // 8. Kirim response tanpa token (karena sudah di-cookie)
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      // Tentukan URL frontend (fallback ke dev jika tidak ada)
      const frontendURL =
...
router.get("/profile", async (req, res) => {
  try {
    ...
    // Ambil nama sekolah jika diperlukan
    let sekolahName = "0";
